...
int main()
{
	...
	cudaMalloc(&D_RES, NIJ*sizeof(double));
	cudaMalloc(&D_LS, NIJ*sizeof(double));
	cudaMalloc(&D_UN, NIJ*sizeof(double));
	cudaMalloc(&D_UE, NIJ*sizeof(double));
	cudaMalloc(&D_LW, NIJ*sizeof(double));
	cudaMalloc(&D_LPR, NIJ*sizeof(double));

	double  *D_U, *D_V, *D_PP, *D_T;

	cudaMalloc(&D_U, NIJ*sizeof(double));
	cudaMalloc(&D_V, NIJ*sizeof(double));
	cudaMalloc(&D_PP, NIJ*sizeof(double));
	cudaMalloc(&D_T, NIJ*sizeof(double));
	...
	for (double ITIM = ITIMS; ITIM < ITIME; ++ITIM)
	{
		...
		for (int ITER = 0; ITER < MAXIT; ++ITER)
		{
			...
			/***************************************************/
			cudaMemcpy(D_LS, LS, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(D_LW, LW, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(D_UN, UN, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(D_UE, UE, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(D_LPR, LPR, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(D_U, U, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			/*******************************************************************************/
			...
			for (int L = 0; L < MAXITU; ++L)
			{
				...
				cudaMemcpy(D_RES, RES, NIJ*sizeof(double), cudaMemcpyHostToDevice);
				...
				cudaMemcpy(U, D_U, NIJ*sizeof(double), cudaMemcpyDeviceToHost);
			...
			cudaMemcpy(D_UE, UE, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(D_LPR, LPR, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(D_V, V, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			...
			for (int L = 0; L < MAXITV; ++L)
			{
				...
				cudaMemcpy(V, D_V, NIJ*sizeof(double), cudaMemcpyDeviceToHost);
			...
			cudaMemcpy(D_UE, UE, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(D_LPR, LPR, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(D_PP, PP, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			...
			for (int L = 0; L < MAXITP; ++L)
			{
				...
				cudaMemcpy(PP, D_PP, NIJ*sizeof(double), cudaMemcpyDeviceToHost);
			...
			cudaMemcpy(D_UE, UE, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(D_LPR, LPR, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(D_T, T, NIJ*sizeof(double), cudaMemcpyHostToDevice);
			...
			for (int L = 0; L < MAXITT; ++L)
			{
				...
				cudaMemcpy(T, D_T, NIJ*sizeof(double), cudaMemcpyDeviceToHost);
